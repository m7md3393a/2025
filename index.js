...
let cmds = {
  ...
  repeat: { cmd: 'loop', a: ['re','تكرار','اعادة'] },
...
// حسن كههربا
        ...
        return msg.channel.send(':white_check_mark: The player has stopped and the queue has been cleared.');
// 04
    } else if(cmd === 'skip') {
...
// 2002
      ...
      if(!queue || !queue.songs) return msg.channel.send(':rolling_eyes: لا توجد موسيقى قيد التشغيل لضبط مستوى الصوت.');

      let vCh = msg.member.voiceChannel;

      if(!vCh || vCh !== msg.guild.me.voiceChannel) return msg.channel.send(':rolling_eyes: أنت لست في قناة صوتي');

      let disp = queue.connection.dispatcher;

      if(isNaN(args[0])) return msg.channel.send(':rolling_eyes: أرقام فقط');
...
// 14-04-2002
      ...
      if(!queue) return msg.channel.send(':sweat: يجب أن يكون هناك تشغيل الموسيقى لاستخدام ذلك!');
...
client.login("NzQ5OTA0NTk0MzM0Nzc3NDM1.X0yxEw.a83w1857uEMp40rT4n560GL9TQM"); 